...
namespace CountableSharp
{
    ...
    {
        private static AnonymousReadOnlyCollection<T> Create<T>(IEnumerable<T> enumerable, int count)
        {
            return new AnonymousReadOnlyCollection<T>(enumerable, count);
        }

        private static AnonymousOrderedReadOnlyCollection<T> CreateOrdered<T>(IOrderedEnumerable<T> enumerable, int count)
        {
        ...
        public static IReadOnlyCollection<TSource> Skip<TSource>(this IReadOnlyCollection<TSource> source, int count)
        {
            return source.Count <= count
                ? Create(Enumerable.Empty<TSource>(), 0)
                : Create(Enumerable.Skip(source, count), source.Count - count);
        }
        ...
        public static IReadOnlyCollection<TSource> Take<TSource>(this IReadOnlyCollection<TSource> source, int count)
        {
            return source.Count <= count
                ? source
                : Create(Enumerable.Take(source, count), count);
        }
